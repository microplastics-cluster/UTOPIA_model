...
def fragmentation(particle):
    ...
    if t_frag_d == "NAN":
        ...
        ):  # print("Smallest sizeBin mp5(0.05um), No fragments formed but it will degrade and is accounted in the discorporation process
            k_frag = 0
            fragments_formed = 0
        else:
            volume_fragment = (
...
def settling(particle):
    ...
    """The influenc of salinity in the settling behaviour should be included as this has been proved for microplastics ()"""
...
def mixing(particle, dict_comp):
    ...
    # k_mix has to be multiplied by the compartment volume ratio calculated with the interacting compartment volume, this is implemented within the fill interaction matrix function
    ...
    #     k_mix = {"mix_up": k_mix_up, "mix_down": k_mix_down}
...
def runoff_transport(particle):
    ...
    k_runoff = runooff_dict[particle.Pcompartment.Cname]
...
def dry_depossition(particle, dict_comp):
    ...
    # Dry depossition rate has to be multiplied by the surface area ratio acording to the recieving compartment. this is implemented within the fill interaction matrix function
