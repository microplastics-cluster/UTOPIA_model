...
def flows_tables_to_csv(
    ...
    writer = pd.ExcelWriter(os.path.join(path, f'{comp}_mass_flows.xlsx'), engine="xlsxwriter")
...
def results_extended_by_compartment_to_csv(path, results_dict):
    # Create a Pandas Excel writer using the file name and desired engine
    writer = pd.ExcelWriter(os.path.join(path, "Results_SS.xlsx"), engine="xlsxwriter")
